...
@app.get("/stars/stats_actor_director_years", response_model=List[schemas.StarActorDirectorStat])
def read_stats_actor_director_years_by_star(mina: Optional[int] = 10, 
        mind: Optional[int] = 10, db: Session = Depends(get_db)):
    return crud.get_stats_by_actor_director(db,mina, mind)
